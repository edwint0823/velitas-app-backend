...
export class OrderService implements IOrderService {
  ...
  async create(orderInfo: createOrderDto): Promise<createOrderResponseDomain> {
    try {
      ...
      const totalPrice = orderInfo.candles.reduce(
        (acc, candle) => acc + candle.price,
        0,
      );
      const totalQuantity = orderInfo.candles.reduce(
        (acc, candle) => acc + candle.quantity,
        0,
      );
      const { value: timeToDoOneCandle } =
        await this.configurationService.findParamByName(
          timeToDoOneCandleNameParam,
        );
      const daysToDoCandles = Math.ceil(
        Math.ceil((totalQuantity * Number(timeToDoOneCandle)) / minutesOfHour) /
          workingHours,
      );

      const maxDayToDelivery = dayjs()
        .set('month', monthMaxDelivery)
        .set('date', dayMaxDelivery);
      let deliveryDate = dayjs().add(
        daysToDoCandles + minimumDaysToDoOrder,
        'days',
      );
      ...
      const statusInfo = await this.statusRepository.findStatusIdByName(
        orderCreateNameStatus,
      );
      if (statusInfo === null) {
      ...
      for (const detail of orderInfo.candles) {
        let candleToPackAlone = detail.name_list.filter(
          (name) => name.packAlone,
        ).length;
        ...
        for (const bag of temporaryBagsNeed) {
          const indexBagsInventoryNeed =
            createOrderInfo.bagInventoryNeed.findIndex(
              (x) => x.bag_id === bag.bag_id,
            );

          if (indexBagsInventoryNeed !== -1) {
            createOrderInfo.bagInventoryNeed[indexBagsInventoryNeed].quantity +=
              bag.quantity;
          } else {
      ...
      const orderCreated =
        await this.orderRepository.createOrder(createOrderInfo);
      return {
